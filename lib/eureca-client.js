...
var EURECA = Class.$extend({
	...
	EURECA.sock = new SockJS(_eureca_uri);
	/*
	...
	*/
	EURECA.sock.onopen = function() {
	    tries = 0;
	};


	EURECA.sock.onmessage = function(e) {
	    try
	    {
	...
	(function(_f) {
		...
		EURECA.sock.send(JSON.stringify({f:_f, a : argsArray, _r : uid}));
	    }
	})(functions[i]);
	}
	this._ready = true;
